...
//        Compiler protocol=ExtensionLoader.getExtensionLoader(Compiler.class).getAdaptiveExtension();
        ...
        URL url = new URL("", "", 0);
        url = url.addParameter("cache", "cache");//添加这段代码 size为11
        ...
        URL urlColor = new URL("", "", 0);
        urlColor = urlColor.addParameter("activate", "activate");//添加这段代码 size为11
        List<Color> colorList = ExtensionLoader.getExtensionLoader(Color.class).getActivateExtension(urlColor,"activate");
