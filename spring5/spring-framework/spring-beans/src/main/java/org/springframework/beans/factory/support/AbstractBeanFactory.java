...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// 获取单例bean Eagerly check singleton cache for manually registered singletons.
		// Eagerly check singleton cache for manually registered singletons.
		// 从缓存中获取单例bean  这里就包括了spring 三级缓存
		...
		if (sharedInstance != null && args == null) { //如果获取到单例bean，则走下面代码
			...
			// 当调用到populateBean方法时，testB会发现依赖testA，那么转过身又会回去创建testA，
			// 当调用到isPrototypeCurrentlyInCreation方法时，此时的testA由于还在进行属性赋值，
			// 并没有完全的创建成功，那么理所当然他会在set集合当中，此时程序会直接抛出异常，告知当前的bean正在创建当中。
			//那么问题来了，如果不抛异常，会怎么样呢？没有这么一个set集合又会怎么样呢？大家可以试想一下.
			// 如果没有这么一个标记，testA在进行属性赋值时，发现依赖testB，马上又去创建testB，到testB进行属性赋值时，又发现依赖了testA，
			// 转过头又去创建testA，而testA依赖了testB，则又会去创建testB。循环往复，形成了一个死循环，永远都出不来了。
			// 所以Spring问了避免这种现象，索性直接抛出异常，因为在Spring看来，它也不知道如何来解决这种情况。
			...
			try {
				...
				//获取这个bean是否有其他的依赖注入放入dependentBeanMap？？？ key是被其他bean依赖的bean  Guarantee initialization of beans that the current bean depends on.
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		//获取bean  Quick check on the concurrent map first, with minimal locking.
