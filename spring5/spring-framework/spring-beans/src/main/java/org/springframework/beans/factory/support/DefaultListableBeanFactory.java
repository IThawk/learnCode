...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public <T> T getBean(Class<T> requiredType) throws BeansException {
		System.out.println("org.springframework.beans.factory.support.DefaultListableBeanFactory.getBean(java.lang.Class<T>)\n " +
				"获取bean：" + requiredType.getName());
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		System.out.println("doGetBeanNamesForType 获取bean的名称");
			...
			if (!isAlias(beanName)) {
				try {
					...
					if (!mbd.isAbstract() && (allowEagerInit ||
						...
						boolean isFactoryBean = isFactoryBean(beanName, mbd);//是否是工厂类
	...
	private <T> NamedBeanHolder<T> resolveNamedBean(
		...
		if (candidateNames.length == 1) {
			...
			System.out.println("new NamedBeanHolder  beanName:"+beanName);
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		if (Optional.class == descriptor.getDependencyType()) {
			...
			//这个地方就是 处理@Lazy的Case  可以解决构造函数的循环依赖问题
			// 通过在构造器参数中标识@Lazy注解，Spring 生成并返回了一个代理对象，因此给Foo注入的Bar并非真实对象而是其代理
			System.out.println("通过在构造器参数中标识@Lazy注解，Spring 生成并返回了一个代理对象 解决循环依赖");
	...
	private class Jsr330Factory implements Serializable {
		...
		private class Jsr330Provider extends DependencyObjectProvider {
