...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 第一次创建beanFactory.并且加载bean definition（xml会在这个地方解析bean定义信息） 定义
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			//预准备 Prepare the bean factory for use in this context.
			prepareBeanFactory(beanFactory);

			try {
				// 后置处理bean 工厂 Allows post-processing of the bean factory in context subclasses.
				postProcessBeanFactory(beanFactory);

				StartupStep beanPostProcess = this.applicationStartup.start("spring.context.beans.post-process");
				// Invoke factory processors registered as beans in the context.
				//执行所有的bean factory进行加强 Processors(进行工厂后置增强)
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				//注册所有的BeanPostProcessors bean的增强器
	...
	protected void prepareRefresh() {
		...
		// 准备环境变量 Validate that all properties marked as required are resolvable:
	...
	protected void initPropertySources() {
		// 执行加载相关信息 For subclasses: do nothing by default.
	}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//刷新bean工厂 注解的时候（org.springframework.context.support.GenericApplicationContext.refreshBeanFactory）
        //xml  org.springframework.context.support.AbstractRefreshableApplicationContext.refreshBeanFactory
		refreshBeanFactory();
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 添加一下后置处理器 Configure the bean factory with context callbacks.
		...
		// 注册默认的bean Register default environment beans.
