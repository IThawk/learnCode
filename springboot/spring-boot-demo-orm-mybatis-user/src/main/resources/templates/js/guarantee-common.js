...
$(function () {
    ...
    if ($('#keyword').length > 0) {
        $('#keyword')
            .bind('focus', function focusKey(e) {
    ...
    // asyncTree 关键字搜索绑定
    if ($('#asyncKeyword').length > 0) {
        $('#asyncKeyword')
            .bind('focus', function focusKey(e) {
                if ($('#asyncKeyword').hasClass('empty')) {
                    $('#asyncKeyword').removeClass('empty')
                }
            })
            .bind('blur', function blurKey(e) {
                if ($('#asyncKeyword').val() === '') {
                    $('#asyncKeyword').addClass('empty')
                }
                $.asyncTree.searchNode(e)
            })
            .bind('input propertychange', $.asyncTree.searchNode)
    }
...
var refreshItem = function () {
    ...
    var target = $('.Guarantee_iframe[data-id="' + currentId + '"]', topWindow)
...
var closeItem = function (dataId) {
    ...
    if ($.common.isNotEmpty(dataId)) {
        ...
        // 移除相应tab对应的内容区
        $('.mainContent .Guarantee_iframe[data-id="' + dataId + '"]', topWindow).remove()
        return
    }
    var panelUrl = window.frameElement.getAttribute('data-panel')
    $('.page-tabs-content .active i', topWindow).click()
    if ($.common.isNotEmpty(panelUrl)) {
        $('.menuTab[data-id="' + panelUrl + '"]', topWindow)
            .addClass('active')
            .siblings('.menuTab')
            .removeClass('active')
        $('.mainContent .Guarantee_iframe', topWindow).each(function () {
            if ($(this).data('id') == panelUrl) {
                $(this).show().siblings('.Guarantee_iframe').hide()
                return false
            }
        })
    }
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        var str =
            '<a href="javascript:;" class="active menuTab noactive" data-id="' +
            dataUrl +
            '" data-panel="' +
            panelUrl +
            '">' +
        ...
        // 添加选项卡对应的iframe
        var str1 =
            '<iframe class="Guarantee_iframe" name="iframe' +
            dataIndex +
            ...
            panelUrl +
            '" seamless></iframe>'
        $('.mainContent', topWindow).find('iframe.Guarantee_iframe').hide().parents('.mainContent').append(str1)
...
function activeWindow() {
    ...
    return $('.Guarantee_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow
}
...
$.ajaxSetup({
    complete: function (XMLHttpRequest, textStatus) {
        if (textStatus == 'parsererror' || textStatus == 'error') {
            let url = XMLHttpRequest.getResponseHeader("redirectUrl");
            if ((url != "")) {
                $.modal.confirm('未登录或登录超时。请重新登录！',(a,b,c)=>{
