...
;(function ($) {
  $.extend({
    ...
    table: {
      ...
      tooltip: function (value, length, target) {
        ...
        if (_value.length > _length) {
          ...
          actions.push(
            $.common.sprintf(
              '<input style="opacity: 0;position: absolute;width:5px;z-index:-1" type="text" value="%s"/>',
              _value
            )
          )
          actions.push(
      ...
      selectDictLabel: function (datas, value) {
        ...
        $.each(datas, function (index, dict) {
          if (dict.dictValue == '' + value) {
            var listClass =
              $.common.equals('default', dict.listClass) || $.common.isEmpty(dict.listClass)
                ? ''
    ...
    operate: {
      ...
      successTabCallback: function (result) {
        if (result.code == web_status.SUCCESS) {
          ...
          var $contentWindow = $('.Guarantee_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow
          $.modal.close()
    ...
    asyncTreeTable: {
      ...
      init: function (options) {
        var defaults = {
          tableId: 'async-tree-table',
          url: '',
          method: 'GET',
          columns: [],
          data: {}, // ajax的参数
          expandColumn: null, // 展开显示的列
          id: 'menuId', // 选取记录返回的值
        ...
        var options = $.extend(defaults, options)
        table.options = options
        table.config[options.tableId] = options
        $.table.initEvent()
        var tableId = options.tableId
        $.btInstance = $('#' + tableId).bootstrapTreeTable({
          tableId: options.tableId, // 选取记录返回的值
          code: options.code, // 用于设置父子关系
          parentCode: options.parentCode, // 用于设置父子关系
          rootCodeValue: options.rootCodeValue, //设置根节点code值----可指定根节点，默认为null,"",0,"0"
          type: options.method, //请求数据的ajax类型
          url: options.url, //请求数据的ajax的url
          ajaxParams: options.data, //请求数据的ajax的data属性
          expandColumn: options.expandColumn, //在哪一列上面显示展开按钮,从0开始
          striped: options.striped, //是否各行渐变色
          bordered: options.bordered, //是否显示边框
          expandAll: options.expandAll, //是否全部展开
          lazyload: options.lazyload, //是否延迟加载
          columns: options.columns, //列数组
          toolbar: options.toolbarId ? '#' + options.toolbarId : null, //顶部工具条
      ...
      setToolbarId: function (toolbarId) {
        $.btInstance.toolbar = toolbarId
      },
      /**
       * 设置在哪一列上面显示展开按钮,从0开始
       */
      setExpandColumn: function (expandColumn) {
        $.btInstance.expandColumn = expandColumn
      },
      /**
       * 设置记录返回的id值
       */
      ...
      setCodeField: function (code) {
        $.btInstance.code = code
      },
      /**
       * 设置记录分级的父级字段
       */
      setParentCodeField: function (parentCode) {
        $.btInstance.parentCode = parentCode
      },
      /**
       * 设置根节点code值----可指定根节点，默认为null,"",0,"0"
       */
      setRootCodeValue: function (rootCodeValue) {
        $.btInstance.rootCodeValue = rootCodeValue
      },
      /**
       * 设置是否默认全部展开
       */
      setExpandAll: function (expandAll) {
        $.btInstance.expandAll = expandAll
      },
      /**
       * 设置是否延迟加载
       */
      setLazyload: function (lazyload) {
        $.btInstance.lazyload = lazyload
      },
      /**
       * 设置表格高度
       */
      setHeight: function (height) {
        $.btInstance.height = height
      },
      /**
      ...
      set: function (key, value) {
        if (typeof key == 'object') {
          for (var i in key) {
            ...
            $.btInstance.data[i] = key[i]
          }
        } else {
          $.btInstance.data[key] = typeof value == 'undefined' ? $('#' + key).val() : value
        }
      },
      /**
       * 设置ajax get请求时候附带的参数
      ...
      search: function(formId) {
        var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
        var params = $.common.formToJSON(currentId);
        $.btInstance.bootstrapTreeTable('refresh', $.extend(params, table.options.ajaxParams));
      },
      refresh: function (params) {
        if (typeof params != 'undefined') {
          $.btInstance.bootstrapTreeTable('refresh', params.query) // 为了兼容bootstrap-table的写法
        } else {
          $.btInstance.bootstrapTreeTable('refresh')
        }
      },
      /**
       * 重置表格视图
      ...
      resetView: function (params) {
        if (typeof params != 'undefined') {
          $.btInstance.bootstrapTreeTable('resetView', params.height) // 为了兼容bootstrap-table的写法
        }
      },
    },
    // 异步树插件封装处理
    asyncTree: {
      ...
      init: function (options) {
        ...
        $.post(options.async.url, options.async.paramData, function (data) {
          var treeId = $('#treeId').val()
          tree = $.fn.zTree.init($('#' + options.id), setting, data)
          $._asyncTree = tree
          for (var i = 0; i < options.expandLevel; i++) {
          ...
          var node = tree.getNodesByParam('id', treeId, null)[0]
          $.asyncTree.selectByIdName(treeId, node)
          // 回调tree方法
          if (typeof options.callBack === 'function') {
            options.callBack(tree)
          }
      ...
      searchNode: function () {
        // 取得输入的关键字的值
        var value = $.common.trim($('#asyncKeyword').val())
        if ($.asyncTree._lastValue == value) {
          return
        }
        // 保存最后一次搜索名称
        $.asyncTree._lastValue = value
        var nodes = $._asyncTree.getNodes()
        // 如果要查空字串，就退出不查了。
        if (value == '') {
          $.asyncTree.showAllNode(nodes)
          return
        }
        $.asyncTree.hideAllNode(nodes)
        // 根据搜索值模糊匹配
        $.asyncTree.updateNodes($._asyncTree.getNodesByParamFuzzy($.asyncTree._option.data.key.name || 'name', value))
      },
      // 根据Id和Name选中指定节点
      selectByIdName: function (treeId, node) {
        if ($.common.isNotEmpty(treeId) && node && treeId == node.id) {
          $._asyncTree.selectNode(node, true)
        }
      },
      // 显示所有节点
      showAllNode: function (nodes) {
        nodes = $._asyncTree.transformToArray(nodes)
        for (var i = nodes.length - 1; i >= 0; i--) {
          if (nodes[i].getParentNode() != null) {
            $._asyncTree.expandNode(nodes[i], true, false, false, false)
          } else {
            $._asyncTree.expandNode(nodes[i], true, true, false, false)
          }
          $._asyncTree.showNode(nodes[i])
          $.asyncTree.showAllNode(nodes[i].children)
        }
      },
      getCheckedNodes: function (column) {
        var _column = $.common.isEmpty(column) ? 'id' : column
        var nodes = $.asyncTree.getCheckedNodes(true)
        console/log
        return $.map(nodes, function (row) {
          return row[_column]
        }).join()
      ...
      // 隐藏所有节点
      hideAllNode: function (nodes) {
        var nodes = $._asyncTree.transformToArray(nodes)
        for (var i = nodes.length - 1; i >= 0; i--) {
          $._asyncTree.hideNode(nodes[i])
        }
      },
      // 显示所有父节点
      showParent: function (treeNode) {
        var parentNode
        while ((parentNode = treeNode.getParentNode()) != null) {
          $._asyncTree.showNode(parentNode)
          $._asyncTree.expandNode(parentNode, true, false, false)
          treeNode = parentNode
        }
      },
      // 显示所有孩子节点
      showChildren: function (treeNode) {
        if (treeNode.isParent) {
          for (var idx in treeNode.children) {
            var node = treeNode.children[idx]
            $._asyncTree.showNode(node)
            $.asyncTree.showChildren(node)
          }
        }
      },
      // 更新节点状态
      updateNodes: function (nodeList) {
        $._asyncTree.showNodes(nodeList)
        for (var i = 0, l = nodeList.length; i < l; i++) {
          var treeNode = nodeList[i]
          $.asyncTree.showChildren(treeNode)
          $.asyncTree.showParent(treeNode)
        }
      },
      // 获取当前被勾选集合
      getCheckedNodes: function (column) {
        var _column = $.common.isEmpty(column) ? 'id' : column
        var nodes = $._asyncTree.getCheckedNodes(true)
        return $.map(nodes, function (row) {
          return row[_column]
        }).join()
      },
      ...
      // 折叠
      collapse: function () {
        $._asyncTree.expandAll(false)
      },
      // 展开
      expand: function () {
        $._asyncTree.expandAll(true)
      },
    },
  })
})(jQuery)
